...
class ViTEmbedding2p5D(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.patch_embed = col_nn.PatchEmbedding2p5D(img_size,
                                                     patch_size,
                                                     in_chans,
                                                     embedding_dim,
                                                     dtype=dtype,
...
class ViTSelfAttention2p5D(nn.Module):
    ...
    def __init__(
        ...
        self.query_key_value = col_nn.Linear2p5D(dim, 3 * dim, dtype=dtype, bias=bias)
        self.attention_dropout = col_nn.Dropout(attention_dropout)
        self.dense = col_nn.Linear2p5D(dim, dim, dtype=dtype, bias=bias)
        self.dropout = col_nn.Dropout(dropout)
...
class ViTMLP2p5D(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.dense_1 = col_nn.Linear2p5D(dim, mlp_ratio * dim, dtype=dtype, bias=bias)
        self.activation = nn.functional.gelu
        self.dropout_1 = col_nn.Dropout(dropout)
        self.dense_2 = col_nn.Linear2p5D(mlp_ratio * dim, dim, dtype=dtype, bias=bias)
...
class ViTBlock2p5D(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.norm1 = col_nn.LayerNorm2p5D(normalized_shape=dim, eps=1e-6, dtype=dtype)
        self.attn = ViTSelfAttention2p5D(dim=dim,
                                         num_heads=num_heads,
                                         attention_dropout=attention_dropout,
                                         dropout=dropout,
                                         bias=bias,
                                         dtype=dtype)
        self.norm2 = col_nn.LayerNorm2p5D(normalized_shape=dim, eps=1e-6, dtype=dtype)
        self.mlp = ViTMLP2p5D(dim=dim, mlp_ratio=mlp_ratio, dropout=dropout, dtype=dtype, bias=bias)
...
class ViTHead2p5D(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.dense = col_nn.Classifier2p5D(dim, num_classes, dtype=dtype, bias=bias)
...
class ViT_Lite_2p5D(nn.Module):
    ...
    def __init__(
        ...
        embed = ViTEmbedding2p5D(img_size=img_size,
        ...
        norm = col_nn.LayerNorm2p5D(normalized_shape=dim, eps=1e-6, dtype=dtype)

        head = ViTHead2p5D(dim=dim, num_classes=num_classes, dtype=dtype, bias=bias)
...
def train_cifar():
    ...
    model = DDP(ViT_Lite_2p5D(), process_group=gpc.get_group(ParallelMode.DATA))

    criterion = col_nn.CrossEntropyLoss2p5D()

    accuracy = col_nn.Accuracy2p5D()
