...
def GAN_training_function(G, D, GD, z_, y_, ema, state_dict, config):
    def train(x, y):
        ...
        if config['toggle_grads']:
            utils.toggle_grad(D.model, True)
            utils.toggle_grad(G.model, False)
        ...
        if config['toggle_grads']:
            utils.toggle_grad(D.model, False)
            utils.toggle_grad(G.model, True)
        ...
        if config['G_ortho'] > 0.0:
            ...
            utils.ortho(G, config['G_ortho'],
                        blacklist=[param for param in G.model.shared.parameters()])
        G.step()
...
def save_and_sample(G, D, G_ema, z_, y_, fixed_z, fixed_y,
    ...
    utils.save_weights(G.model, D.model, state_dict, config['weights_root'],
    ...
    if config['num_save_copies'] > 0:
        utils.save_weights(G.model, D.model, state_dict, config['weights_root'],
                           ...
                           G_ema.model if config['ema'] else None)
        state_dict['save_num'] = (
    ...
    with torch.no_grad():
        if config['parallel']:
            ...
            fixed_Gz = which_G(fixed_z, which_G.model.shared(fixed_y))
