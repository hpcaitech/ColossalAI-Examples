...
def parse_args():
    ...
    parser.add_argument("--init_in_cpu", action='store_true',
                        default=False, help="init training model in cpu")
...
def main():
    ...
    if args.seed is not None:
        set_seed(args.seed)
        logger.info(
            f"Rank {dist.get_rank()}: random seed is set to {args.seed}")
    ...
    logger.info(
        f"  Instantaneous batch size per device = {args.per_device_train_batch_size}", ranks=[0])
    logger.info(
        f"  Total train batch size (w. parallel, distributed & accumulation) = {total_batch_size}", ranks=[0])
    logger.info(
        f"  Gradient Accumulation steps = {args.gradient_accumulation_steps}", ranks=[0])
    logger.info(
        f"  Total optimization steps = {args.max_train_steps}", ranks=[0])
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            global_step += 1
            logger.info("Global step {} finished".format(
                global_step + 1), ranks=[0])
        ...
        logger.info(
            f"Epoch {epoch}: perplexity: {perplexity} eval_loss: {eval_loss}", ranks=[0])

    if args.output_dir is not None:
        model_state = model.state_dict()
        if is_main_process:
            torch.save(model_state, args.output_dir +
                       '/epoch_{}_model.pth'.format(completed_steps))
        dist.barrier()
